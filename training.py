...
def process(paths):
	...
	# Train the model
	run_hist_2=model.model.fit_generator(generator = train,steps_per_epoch = 3,epochs = 5,verbose = 1,validation_data = valid,callbacks = callback)
	model.model.save("tr.h5")
	
	fig = plt.figure()
	plt.plot(run_hist_2.history["loss"],'r', marker='.', label="Training Loss")
	...
	plt.legend()
	plt.grid()
	fig.savefig('Training Loss vs Validation Loss.jpg') 
	plt.pause(5)
